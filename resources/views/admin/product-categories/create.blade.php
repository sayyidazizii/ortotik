...
@section('header_title', 'Tambah Kategori Produk')

@section('content')
<div class="max-w-2xl space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-black text-slate-900">Tambah Kategori Produk Baru</h2>
            <p class="text-xs text-slate-500 mt-0.5">Kategori ini akan langsung muncul sebagai opsi filter di e-katalog produk website.</p>
        </div>
    ...
    <form action="{{ route('admin.product-categories.store') }}" method="POST" class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8 space-y-6">
        @csrf
        ...
        <div class="space-y-4">
            <div class="space-y-1.5">
                ...
                <input type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: Kaki Palsu (Prostesis Bawah)" required
            ...
            <div class="space-y-1.5">
                <label class="block text-xs font-bold text-slate-700 uppercase">Slug URL (Opsional)</label>
                <input type="text" name="slug" value="{{ old('slug') }}" placeholder="Contoh: kaki-palsu-prostesis (otomatis jika dikosongkan)"
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="description" rows="3" placeholder="Penjelasan singkat mengenai ragam alat dalam kategori ini..."
                          class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-medical-500 focus:outline-none">{{ old('description') }}</textarea>
            </div>
        </div>

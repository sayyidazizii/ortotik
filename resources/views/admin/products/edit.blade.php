...
@section('header_title', 'Edit Produk E-Katalog')

@section('content')
@php
    $currentThumb = $product->thumbnail ?? '';
    ...
    if (is_array($product->specifications)) {
        ...
        $currentSpec = $product->specifications ?? '';
    }

    // Format existing gallery images
    $existingGallery = [];
    if (is_array($product->images)) {
        foreach ($product->images as $img) {
            if (!empty($img)) {
                $src = str_starts_with($img, 'http') ? $img : asset(ltrim($img, '/'));
                $existingGallery[] = [
                    'raw' => $img,
                    'src' => $src
                ];
...
<div class="max-w-4xl space-y-6" x-data="productEditManager()">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('products.show', $product->slug) }}" target="_blank" class="text-xs font-bold text-medical-600 hover:text-medical-700 inline-flex items-center gap-1">
    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm space-y-8">
        @csrf
        @method('PUT')

        <!-- SECTION: Foto Utama & Galeri Multi-Image -->
        <div class="space-y-6">
            <div class="border-b border-slate-100 pb-3">
                ...
                <p class="text-xs text-slate-500 mt-0.5">Kelola foto sampul dan album foto galeri produk untuk slider di halaman publik.</p>
            </div>

            <!-- 1. FOTO UTAMA (THUMBNAIL) -->
            <div class="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-4">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-[11px] text-slate-400 font-medium">Foto yang tampil pertama & di kartu e-katalog</span>
                </div>
                ...
                <div class="flex flex-col sm:flex-row items-start gap-5">
                    ...
                    <div class="flex-1 space-y-3 w-full">
                        <div>
                            ...
                            <p class="text-[11px] text-slate-400 mt-1">Kosongkan jika tidak ingin mengganti file foto sampul saat ini.</p>
                        </div>
            ...
            <!-- 2. GALERI FOTO TAMBAHAN (MULTI-IMAGE) -->
            <div class="p-5 rounded-2xl bg-indigo-50/50 border border-indigo-100 space-y-5">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <div class="flex items-center gap-2 text-indigo-700">
                        <i data-lucide="sliders" class="w-4 h-4"></i>
                        <h4 class="text-xs font-bold uppercase tracking-wider text-slate-800">2. Galeri Foto Tambahan Produk</h4>
                    </div>
                    <span class="text-[11px] text-indigo-600 font-medium">Kelola foto-foto sudut lain untuk slider</span>
                </div>

                <!-- Foto Galeri yang Tersimpan Saat Ini -->
                <template x-if="retainedGallery.length > 0">
                    <div class="space-y-2">
                        <span class="text-xs font-bold text-slate-700 block">Foto Galeri Saat Ini (<span x-text="retainedGallery.length"></span> Foto):</span>
                        <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-3">
                            <template x-for="(item, gIdx) in retainedGallery" :key="gIdx">
                                <div class="relative group rounded-xl overflow-hidden border border-slate-200 bg-white aspect-square shadow-xs p-2 flex items-center justify-center">
                                    ...
                                    <button type="button" @click="removeExistingImage(gIdx)" class="absolute top-1 right-1 p-1 rounded-md bg-rose-600 text-white hover:bg-rose-700 transition shadow-sm" title="Hapus foto dari galeri">
                                        <i data-lucide="trash-2" class="w-3.5 h-3.5"></i>
                                    </button>
                                    <div class="absolute bottom-1 left-1 bg-black/60 text-white text-[9px] px-1.5 py-0.5 rounded font-mono" x-text="'Foto ' + (gIdx + 2)"></div>
                                    <input type="hidden" name="retained_gallery_images[]" :value="item.raw">
                                </div>
                            </template>
                        </div>
                    </div>
                ...
                <!-- Multi-File Upload Baru -->
                <div class="space-y-3 pt-3 border-t border-indigo-100">
                    <label class="block text-xs font-bold text-slate-700">Upload Tambahan Foto Baru ke Galeri</label>
                    <input type="file" 
                           name="gallery_files[]" 
                           multiple 
                           accept="image/jpeg,image/png,image/webp,image/svg+xml"
                           @change="handleNewGalleryFiles($event)"
                    ...
                    <p class="text-[11px] text-slate-400">Pilih beberapa file sekaligus dengan menahan <kbd class="px-1.5 py-0.5 bg-slate-200 rounded text-[10px] font-mono">Ctrl</kbd> atau <kbd class="px-1.5 py-0.5 bg-slate-200 rounded text-[10px] font-mono">Shift</kbd>.</p>
                </div>
                ...
                <template x-if="newGalleryPreviews.length > 0">
                    <div class="space-y-2 pt-2 border-t border-indigo-100">
                        <span class="text-xs font-bold text-slate-700 block">Preview Foto Baru yang Akan Diupload (<span x-text="newGalleryPreviews.length"></span> Foto):</span>
                        <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-3">
                            <template x-for="(np, nIdx) in newGalleryPreviews" :key="nIdx">
                                <div class="relative group rounded-xl overflow-hidden border border-emerald-300 bg-white aspect-square shadow-xs p-2 flex items-center justify-center">
                                    <img :src="np.src" class="w-full h-full object-contain">
                                    <button type="button" @click="removeNewGalleryPreview(nIdx)" class="absolute top-1 right-1 p-1 rounded-md bg-black/60 text-white hover:bg-rose-600 transition" title="Batal upload foto ini">
                                        <i data-lucide="x" class="w-3 h-3"></i>
                                    </button>
                                    <span class="absolute top-1 left-1 bg-emerald-600 text-white text-[8px] font-bold px-1 rounded">Baru</span>
                                </div>
                            </template>
                        </div>
                    </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" name="sku" value="{{ old('sku', $product->sku) }}"
                    class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-medical-500 font-mono">
            </div>

            <!-- Price -->
            <div class="space-y-1.5">
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider">Estimasi Harga (Rp)</label>
                <input type="number" name="price" value="{{ old('price', $product->price) }}" placeholder="Contoh: 4500000"
                    class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-medical-500">
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <input type="number" name="discount_price" value="{{ old('discount_price', $product->discount_price) }}" placeholder="Kosongkan jika tidak diskon"
                    class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-medical-500">
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <select name="stock_status" required class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-medical-500">
                    <option value="ready_stock" {{ old('stock_status', $product->stock_status) === 'ready_stock' || old('stock_status', $product->stock_status) === 'in_stock' ? 'selected' : '' }}>Ready Stock</option>
                    <option value="pre_order" {{ old('stock_status', $product->stock_status) === 'pre_order' ? 'selected' : '' }}>Pre Order</option>
                    <option value="custom_only" {{ old('stock_status', $product->stock_status) === 'custom_only' ? 'selected' : '' }}>Custom Made Only</option>
                </select>
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" name="warranty_period" value="{{ old('warranty_period', $product->warranty_period ?? '1 Tahun Garansi Fitting & Frame') }}"
                    class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-medical-500">
            </div>
        </div>
        ...
        <div class="space-y-1.5">
            ...
            <textarea name="short_description" rows="2" required
                class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-medical-500">{{ old('short_description', $product->excerpt ?? $product->short_description) }}</textarea>
        </div>
        ...
        <div class="space-y-1.5">
            ...
            <textarea name="description" rows="5" required
                class="wysiwyg-editor w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-medical-500">{{ old('description', $product->description) }}</textarea>
        </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="mt-1.5">
                        <textarea name="medical_indication" rows="4" placeholder="Cedera ACL / PCL&#10;Instabilitas Sendi Lutut Berat..."
                            class="wysiwyg-editor w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-medical-500 bg-white">{{ old('medical_indication', $product->medical_indications ?? $product->medical_indication) }}</textarea>
                    </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="mt-1.5">
                        <textarea name="material_spec" rows="4" placeholder="Carbon Composite & Aluminium Aircraft Grade..."
                            class="wysiwyg-editor w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-medical-500 bg-white">{{ old('material_spec', $currentSpec) }}</textarea>
                    </div>
            ...
            <div class="space-y-2 pt-6 border-t border-slate-200">
                ...
                <div class="mt-1.5">
                    <textarea name="size_chart" rows="3" placeholder="S: Lingkar lutut 30 - 35 cm&#10;M: Lingkar lutut 35 - 40 cm&#10;L: Lingkar lutut 40 - 45 cm"
                        class="wysiwyg-editor w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-medical-500 bg-white">{{ old('size_chart', $product->size_chart) }}</textarea>
                </div>
        ...
        <div class="mt-8 flex flex-wrap gap-6 pt-6 border-t border-slate-200 text-xs font-bold text-slate-700">
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $product->is_active) ? 'checked' : '' }} class="w-4 h-4 text-medical-600 rounded">
                <span>Tampilkan di Katalog Publik (Aktif)</span>
            </label>

            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $product->is_featured) ? 'checked' : '' }} class="w-4 h-4 text-medical-600 rounded">
                <span>Jadikan Produk Unggulan di Beranda</span>
            </label>

            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_custom_order" value="1" {{ old('is_custom_order', $product->is_custom_order) ? 'checked' : '' }} class="w-4 h-4 text-medical-600 rounded">
...
function productEditManager() {
    return {
        ...
        thumbnailUrl: '{{ old('thumbnail', $product->thumbnail ?? '') }}',
        retainedGallery: @json($existingGallery),
        newGalleryPreviews: [],
        selectedPresetUrls: [],
        ...
        removeExistingImage(idx) {
            this.retainedGallery.splice(idx, 1);
        },

        handleNewGalleryFiles(e) {
        ...
        removeNewGalleryPreview(idx) {
            this.newGalleryPreviews.splice(idx, 1);
        },

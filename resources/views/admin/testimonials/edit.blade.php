...
@section('title', 'Edit Testimoni Pasien')
@section('header_title', 'Edit Testimoni Pasien')

@section('content')
<div class="max-w-2xl space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-black text-slate-900">Edit Testimoni Pasien</h2>
            <p class="text-xs text-slate-500 mt-0.5">Ubah data ulasan dari pasien: <span class="font-bold text-slate-800">{{ $testimonial->patient_name }}</span></p>
        </div>
    ...
    <form action="{{ route('admin.testimonials.update', $testimonial->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="patient_name" value="{{ old('patient_name', $testimonial->patient_name) }}" required
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="patient_info" value="{{ old('patient_info', $testimonial->patient_info) }}" placeholder="Contoh: Sleman • Pasien Kaki Palsu"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="service_used" value="{{ old('service_used', $testimonial->service_used) }}" required
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-medical-500 focus:outline-none">
                </div>
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="testimony" rows="4" required
                          class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-medical-500 focus:outline-none leading-relaxed">{{ old('testimony', $testimonial->testimony) }}</textarea>
            </div>
            ...
            <div class="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-3">
                <div class="flex items-center gap-4">
                    ...
                    <div class="w-12 h-12 rounded-full overflow-hidden border border-slate-300 shrink-0 bg-white">
                        <img src="{{ asset($testimonial->photo) }}" alt="{{ $testimonial->patient_name }}" class="w-full h-full object-cover">
                    </div>
                    @endif
                    <div class="space-y-1 flex-1">
                        <label class="block text-xs font-bold text-slate-700 uppercase">Upload Foto Pasien Baru (Opsional)</label>
                ...
                <input type="text" name="photo" value="{{ old('photo', $testimonial->photo) }}" placeholder="Atau link URL foto..."
                       class="w-full px-3.5 py-2 rounded-xl border border-slate-200 text-xs bg-white">
            </div>

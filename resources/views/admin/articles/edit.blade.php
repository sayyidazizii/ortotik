...
<div class="max-w-4xl space-y-6" x-data="{
    thumbnailUrl: '{{ old('thumbnail', $article->thumbnail) }}',
    filePreview: '',
    get displayPreview() {
        if (this.filePreview) return this.filePreview;
        if (!this.thumbnailUrl) return '';
        if (!this.thumbnailUrl.startsWith('http') && !this.thumbnailUrl.startsWith('/')) {
            return '{{ asset('') }}' + this.thumbnailUrl.replace(/^\/+/, '');
        }
    ...
    handleFileSelect(e) {
        ...
        // Kosongkan URL agar foto file baru yang diutamakan
        this.thumbnailUrl = '';
    },
    clearFile() {
        this.filePreview = '';
    ...
    <form action="{{ route('admin.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-4">
            <div class="flex items-center gap-2 text-medical-600">
                ...
                <h3 class="text-xs font-bold uppercase tracking-wider text-slate-800">Upload / Ganti Foto Sampul Artikel</h3>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-5">
                ...
                <div class="flex-1 space-y-3 w-full">
                    <div>
                        ...
                        <p class="text-[11px] text-slate-400 mt-1">Format: JPG, PNG, WEBP (Maks. 10MB). Kosongkan jika tidak ingin mengubah foto sampul.</p>
                    </div>
        ...
        <div class="space-y-1.5">
            ...
            <textarea name="content" rows="12" required
                class="wysiwyg-editor w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-medical-500">{{ old('content', $article->content) }}</textarea>
        </div>
